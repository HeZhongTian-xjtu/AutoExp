"""Offline Online Shoppers binary classification entrypoint."""
...
def _load_dataset() -> tuple[pd.DataFrame, dict]:
    ...
    if 'Revenue' not in frame.columns:
        raise ValueError('the Online Shoppers asset must contain Revenue')
...
def main(smoke: bool = False) -> None:
    ...
    features = frame.drop(columns=['Revenue'])
    target = frame['Revenue'].astype(bool).astype(int)
    ...
    metadata = {
        ...
        'details': {'target': 'Revenue', 'positive_label': True, 'split': '80/20 stratified split by seed', 'feature_count': int(features.shape[1])},
    }
